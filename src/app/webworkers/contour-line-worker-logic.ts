...
export class ContourLineWorkerLogic {
    ...
    public generateContourLine(request: ContourLineRequest): ContourLineResponse {
        ...
        for (let x = 0; x < request.bodyLength; x++) {
            yValues.push(this.nextY(request.seed, x * request.stretching, request.heightAmplitude));
        }
        const seamFactor = 1.0 / request.seamLength;
        for (let x = 0; x < request.seamLength; x++) {
            const y = ContourLineWorkerLogic.lerp(
                this.nextY(request.seed, (x + request.bodyLength) * request.stretching, request.heightAmplitude),
                yValues[x],
                x * seamFactor);
            yValues[x] = y;
            yValues.push(y);
    ...
    private nextY(seed: number, distance: number, amplitude: number): number {
        return (this.noise2D(distance, seed) * amplitude) + amplitude;
    }
}
